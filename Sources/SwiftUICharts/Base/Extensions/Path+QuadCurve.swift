...
extension Path {
    ...
    static func quadCurvedPathWithPoints(points: [Double], step: CGPoint, paddingBottom: Double = 0) -> Path {
        var path = Path()
        if points.count < 2 {
            return path
        }
        guard let offset = points.min() else { return path }
        let allSame = points.min() == points.max()
        var point1 = CGPoint(x: 0, y: allSame ? step.y : CGFloat(paddingBottom) + CGFloat(points[0]-offset) * step.y)
        path.move(to: point1)
        for pointIndex in 1..<points.count {
            let point2 = CGPoint(x: step.x * CGFloat(pointIndex), y: allSame ? step.y : CGFloat(paddingBottom) + step.y * CGFloat(points[pointIndex]-offset))
    ...
    static func quadClosedCurvedPathWithPoints(points: [Double], step: CGPoint, paddingBottom: Double = 0) -> Path {
        var path = Path()
        if points.count < 2 {
            return path
        }
    ...
    static func linePathWithPoints(points: [Double], step: CGPoint, paddingBottom: Double = 0) -> Path {
        var path = Path()
        if points.count < 2 {
            return path
        }
        guard let offset = points.min() else { return path }
        let allSame = points.min() == points.max()
        let point1 = CGPoint(x: 0, y: allSame ? step.y : CGFloat(paddingBottom) + CGFloat(points[0]-offset) * step.y)
        path.move(to: point1)
        for pointIndex in 1..<points.count {
            let point2 = CGPoint(x: step.x * CGFloat(pointIndex), y: allSame ? step.y : CGFloat(paddingBottom) + step.y * CGFloat(points[pointIndex]-offset))
    ...
    static func closedLinePathWithPoints(points: [Double], step: CGPoint, paddingBottom: Double = 0) -> Path {
        var path = Path()
        if points.count < 2 {
            return path
        }
        ...
        path.move(to: point1)
        for pointIndex in 1..<points.count {
            point1 = CGPoint(x: step.x * CGFloat(pointIndex), y: allSame ? step.y : CGFloat(paddingBottom) + step.y * CGFloat(points[pointIndex]-offset))
            path.addLine(to: point1)
        }
        path.addLine(to: CGPoint(x: point1.x, y: 0))
        path.closeSubpath()
